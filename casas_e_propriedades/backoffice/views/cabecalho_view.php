<?php 
  ...
  if(!empty($_POST["acao"]) && !empty($_POST["ids"])){
    ...
    if($acao === "apagar"){
      ...
      idu_sql("UPDATE carousel SET activo = 'Sim' WHERE id IN ($ids_str)");
    }elseif($acao === "desativar"){
      idu_sql("UPDATE carousel SET activo = 'Não' WHERE id IN ($ids_str)");
    }

    header("Location: cabecalho.php?saved=1");
    exit;
  }
  ...
  <main class="container">
    <div class="row mt-5">
      <div class="col-12 px-0">
        ...
        <p class=""><span>Imagens do Cabeçalho do Site</span> Listagem de todas as Imagens.</p>
        <hr class="mt-1 mb-1">
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12 px-0">
        <button class="popup" onclick="abrir_popup('cabecalho/cabecalho-add.php')">NOVO</button>
        <hr class="mt-1">
      </div>
        ...
        <form action="" method="post">
          <table class="m-auto w-100">
            ...
            <?php foreach($carousel as $c): ?>
              <tr>
                <td><input type="checkbox" name="ids[]" value="<?= $c["id"] ?>" class="checkbox"></td>
                <td><?= $c["id"] ?></td>
                <td><img src="<?= $c["imagem"] ?>" class="imagem" alt="erro"></td>
                <td><?= $c["titulo"] ?></td>
                <td><?= $c["subtitulo"] ?></td>
                <td><?= $c["tag"] ?></td>
                <td><?= $c["activo"] ?></td>
                <td><?= $c["posicao"] ?></td>
                <td><button type="button" class="popup" onclick="abrir_popup('cabecalho/cabecalho-edit.php?id=<?= $c['id'] ?>')">Editar</button>|<a href="cabecalho/cabecalho-delete.php?id=<?= $c['id'] ?>" onclick="return confirm('Esta ação é irreversível, deseja continuar?')">Excluir</a></td>
              </tr>
            <?php endforeach ?>
