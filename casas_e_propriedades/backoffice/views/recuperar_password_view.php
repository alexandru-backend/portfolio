  ...
  if($_SERVER["REQUEST_METHOD"] === "POST"){
    ...
    if(empty($login) || empty($senha) || empty($confirm_senha)){
        ...
        if(!$utilizador){
            ...
            $stmt = $pdo->prepare("UPDATE backoffice SET senha = ? WHERE login = ?");
            $stmt->execute([$hash, $login]);

            $sucesso = "Senha atualizada com sucesso! Agora pode fazer login.";
        }
    }
  }
?>
  ...
  <main class="container">
    <div class="row mt-5 mb-">
      <div class="col-12 px-0">
        ...
        <p class="mb-0"><span>Recuperar senha</span> Recupere senha abaixo.</p>
        <hr class="my-1">
