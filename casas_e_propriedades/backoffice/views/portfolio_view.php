<?php
  ...
  if(!empty($_POST["acao"]) && !empty($_POST["ids"])){
    ...
    if($acao === "apagar"){
      ...
      idu_sql("UPDATE portfolio SET activo = 'Sim' WHERE id IN ($ids_str)");
    }elseif($acao === "desativar"){
      idu_sql("UPDATE portfolio SET activo = 'Não' WHERE id IN ($ids_str)");
    }
...
<main class="container">
    <div class="row mt-5">
      <div class="col-12 px-0">
        ...
        <p class=""><span>Portfólio</span> Listagem de todas os Portfólios.</p>
        <hr class="mt-1">
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12 px-0">
        <button class="popup" onclick="abrir_popup('portfolio/portfolio-add.php')">NOVO</button>
        <hr class="mt-1">
      </div>
        ...
        <form action="" method="post">
          <table class="m-auto w-100">
            ...
            <?php foreach($portfolio as $i => $p):                       
              $imagens_data = get_portfolio_imagens($p["id"]);                      
            ...
            ?>
              <tr>
                ...
                <td><?= $p["activo"] ?></td>
                <td><?= $p["posicao"] ?></td>
                <td><button type="button" class="popup" onclick="abrir_popup('portfolio/portfolio-edit.php?id=<?= $p['id'] ?>')">Editar</button>|<a href="portfolio/portfolio-delete.php?id=<?= $p['id'] ?>" onclick="return confirm('Esta ação é irreversível, deseja continuar?')">Excluir</a></td>
              </tr>
