<?php
  ...
  if(!empty($_POST["acao"]) && !empty($_POST["ids"])){
    ...
    if($acao === "apagar"){
      ...
      idu_sql("UPDATE predios SET activo = 'Sim' WHERE id IN ($ids_str)");
    }elseif($acao === "desativar"){
      idu_sql("UPDATE predios SET activo = 'Não' WHERE id IN ($ids_str)");
    }

    header("Location: predios.php?saved=1");
    exit;
  }
?>
<main class="container">
    <div class="row mt-5">
      <div class="col-12 px-0">
        ...
        <p class=""><span>Tipos de Fogo</span> Listagem de todos os Projectos em Comercialização Tipos de Fogo.</p>
        <hr class="mt-1">
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12 px-0">
        <button class="popup" onclick="abrir_popup('predios/predios-add.php')">NOVO</button>
        <hr class="mt-1">
      </div>
        ...
        <form action="" method="post">
          <table class="m-auto w-100">
            <tr>
              ...
              <th>ID</th>
              <th>IMAGEM</th>
              <th>PROJECTO EM COMERCIALIZAÇÃO</th>
              <th>OBSERVAÇÃO</th>
              <th>POSIÇÃO</th>
              <th>ACTIVO</th>
              <th>AÇÕES</th>
            ...
            <?php foreach($predios as $i => $p): 
              $imagens_data = get_predio_imagens($p["id"]);                      
              $imagens = json_decode($imagens_data["imagens"]); 
              $img = $imagens[0];?>
              <tr>
                <td><input type="checkbox" name="ids[]" value="<?= $p["id"] ?>" class="checkbox"></td>
                <td><?= $p["id"] ?></td>
                <td><img src="<?= $img ?>" class="imagem" alt="erro"></td>
                <td><?= $p["nome"] ?></td>
                <td><?= $p["tipo"] ?></td>
                <td><?= $p["posicao"] ?></td>
                <td><?= $p["activo"] ?></td>
                <td><button type="button" class="popup" onclick="abrir_popup('predios/predios-edit.php?id=<?= $p['id'] ?>')">Editar</button>|<a href="predios/predios-delete.php?id=<?= $p['id'] ?>" onclick="return confirm('Esta ação é irreversível, deseja continuar?')">Excluir</a></td>
              </tr>
